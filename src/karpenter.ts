...
export class Karpenter extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    const accessEntry = pulumi
      .output(args.createAccessEntry)
      .apply((createAccessEntry) => {
        if (createAccessEntry ?? true) {
          return new aws.eks.AccessEntry(
            name,
            {
    ...
    this.nodeRoleName = pulumi
      .output(accessEntry)
      .apply((_) => getRoleName(nodeRoleArn));

    const controllerRoleArn = this.createControllerRole(
      name,
      args,
      nodeRoleArn,
    );
    ...
    const serviceAccount = pulumi
      .output(args.serviceAccount)
      .apply((sa) => sa ?? 'karpenter');
  ...
  createControllerRole(
    ...
    const controllerPolicy = new aws.iam.Policy(
      `${name}-controller-policy`,
      {
        ...
        tags: pulumi
          .all([args.tags, args.controllerRoleArgs?.tags])
          .apply(([tags, controllerRoleTags]) => ({
            ...tags,
            ...controllerRoleTags,
          })),
      },
    ...
    const role = new awsNative.iam.Role(
      `${name}-controller-role`,
      {
        ...
        managedPolicyArns: pulumi
          .output(args.controllerRoleArgs?.additionalManagedPolicyArns)
          .apply((additionalManagedPolicyArns) => [
            ...(additionalManagedPolicyArns ?? []),
            controllerPolicy.arn,
          ]),

        tags: pulumi
          .all([args.tags, args.controllerRoleArgs?.tags])
          .apply(
            ([tags, controllerRoleTags]) =>
              toNativeTags({ ...tags, ...controllerRoleTags })!,
          ),
      },
      { parent: this },
    );
  ...
  createNodeRole(name: string, args: KarpenterArgs): pulumi.Output<string> {
    ...
    const managedPolicyArns = [
      ...
      (policyName) =>
        pulumi.interpolate`arn:${partition}:iam::aws:policy/${policyName}`,
    );
    ...
    const ipv6Policy = cluster.kubernetesNetworkConfigs.apply(
    ...
    const role = new awsNative.iam.Role(
      `${name}-node-role`,
      {
        ...
        tags: pulumi
          .all([args.tags, args.nodeRoleArgs?.tags])
          .apply(
            ([tags, nodeRoleTags]) =>
              toNativeTags({ ...tags, ...nodeRoleTags })!,
          ),
      },
      { parent: this },
    );
  ...
  createSqsQueue(
    name: string,
    args: QueueArgs | undefined,
    parentTags: Tags | undefined,
  ): aws.sqs.Queue {
    if (
      args?.managedSseEnabled &&
      (args?.kmsMasterKeyId || args?.kmsDataKeyReusePeriodSeconds)
    ) {
      throw new pulumi.InputPropertyError({
        propertyPath: 'queue.managedSseEnabled',
        reason:
          'kmsMasterKeyId and kmsDataKeyReusePeriodSeconds must not be set if managedSseEnabled is true',
      });
    }

    const sqsManagedSseEnabled = pulumi
      .all([
        args?.managedSseEnabled,
        args?.kmsMasterKeyId,
        args?.kmsDataKeyReusePeriodSeconds,
      ])
      .apply(
        ([managedSseEnabled, kmsMasterKeyId, kmsDataKeyReusePeriodSeconds]) => {
  ...
  createSqsQueuePolicy(
    name: string,
    queue: aws.sqs.Queue,
  ): aws.sqs.QueuePolicy {
    return new aws.sqs.QueuePolicy(
      name,
      {
        queueUrl: queue.url,
  ...
  nodeTerminationEventRules(
    name: string,
  ): pulumi.Output<{ ruleArn: string; targetArn: string }>[] {
    const events = {
      HealthEvent: {
      ...
      SpotInterrupt: {
        description:
          'Karpenter interrupt - EC2 spot instance interruption warning',
        eventPattern: {
          source: ['aws.ec2'],
      ...
      InstanceRebalance: {
        description:
          'Karpenter interrupt - EC2 instance rebalance recommendation',
        eventPattern: {
          source: ['aws.ec2'],
      ...
      InstanceStateChange: {
        description:
          'Karpenter interrupt - EC2 instance state-change notification',
        eventPattern: {
          source: ['aws.ec2'],
    ...
    return Object.entries(events).map(([key, event]) => {
      ...
      return pulumi
        .all([rule.arn, target.arn])
        .apply(([ruleArn, targetArn]) => ({
          ruleArn,
          targetArn,
        }));
    });
    ...
    return aws.iam.getPolicyDocumentOutput(
      {
        statements: [
          ...
          },
          {
            ...
            effect: 'Allow',
            resources: [
              pulumi.interpolate`arn:${partition}:ssm:${region}::parameter/aws/service/*`,
            ],
          ...
          },
          {
            ...
            effect: 'Allow',
            resources: [
              pulumi.interpolate`arn:${partition}:iam::*:instance-profile/*`,
            ],
          ...
          },
          {
            ...
            effect: 'Allow',
            resources: [
              pulumi.interpolate`arn:${partition}:eks:${region}:*:cluster/${args.clusterName}`,
            ],
