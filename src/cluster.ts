 ...
 */
export type AddonConfiguration = Omit<aws.eks.AddonArgs, "clusterName" | "addonName" | "configurationValues"> & {
    /**
     * Configuration values to apply to the addon.
     * These values override the default configuration provided by AWS.
     */
...
export class Cluster extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: ClusterArgs, opts?: pulumi.ComponentResourceOptions) {
        ...
        const clusterRoleArn = this.getClusterRoleArn(name, args, partition, encryptionKeyArn);
        ...
        this.cluster = new aws.eks.Cluster(name, {
            ...
            vpcConfig: {
                ...
                endpointPrivateAccess: pulumi.output(args.vpcConfig.apiServerEndpoints).apply(endpoints => endpoints?.includes("private") ?? false),
                endpointPublicAccess: pulumi.output(args.vpcConfig.apiServerEndpoints).apply(endpoints => endpoints?.includes("public") ?? true),
            ...
            kubernetesNetworkConfig: {
                elasticLoadBalancing: {
                    enabled: pulumi.output(args.autoMode?.enabled).apply(enabled => enabled ?? true),
                },
                ipFamily: args.networkConfig?.ipFamily,

                // those casts actually work, I promise (fingers crossed)
                serviceIpv4Cidr: args.networkConfig?.serviceCidr ? pulumi.all([args.networkConfig.serviceCidr, args.networkConfig?.ipFamily]).apply(([serviceCidr, ipFamily]) => ipFamily === "ipv4" ? serviceCidr : undefined) as pulumi.Output<string> : undefined,
                serviceIpv6Cidr: args.networkConfig?.serviceCidr ? pulumi.all([args.networkConfig.serviceCidr, args.networkConfig?.ipFamily]).apply(([serviceCidr, ipFamily]) => ipFamily === "ipv6" ? serviceCidr : undefined) as pulumi.Output<string> : undefined,
            },
            ...
            computeConfig: {
                enabled: pulumi.output(args.autoMode?.enabled).apply(enabled => enabled ?? true),
                nodePools: pulumi.output(args.autoMode?.nodePools).apply(nodePools => nodePools ?? ["general-purpose", "system"]),
        ...
        }, { parent: this }).issuerArn;

        const clusterCreatorAdmin = this.createClusterAdmins(`${name}-cluster-creator-admin`, clusterCreator, partition, args.tags);
        ...
        const addons = pulumi.output(args.addons).apply(addons => {
            return Object.entries(addons ?? {}).map(([addonName, args]) => {
                ...
                return new aws.eks.Addon(`${name}-${addonName}`, {
                    ...addonArgs,
                    clusterName: this.cluster.name,
                    addonName: addonName,
                    addonVersion: version,
                    configurationValues: args.configurationValues ? pulumi.jsonStringify(args.configurationValues) : undefined,
                    tags: args.tags,
                }, { parent: this });
            });
        });

        this.installedAddons = pulumi.output(addons.apply(addons => addons.map(addon => addon.addonName)));
    ...
    getAutoModeRoleArn(name: string, args: ClusterArgs, partition: pulumi.Output<string>): pulumi.Input<string> | undefined {
        return pulumi.output(args.autoMode?.enabled).apply(enabled => {
            ...
            const role = new awsNative.iam.Role(`${name}-auto-mode-role`, {
    ...
    getClusterRoleArn(name: string, args: ClusterArgs, partition: pulumi.Output<string>, encryptionKeyArn: pulumi.Input<string>): pulumi.Input<string> {
        if (args.roleArn) {
            return args.roleArn;
        }

        const role = new awsNative.iam.Role(`${name}-cluster-role`, {
            assumeRolePolicyDocument: {
                Version: "2012-10-17",
                Statement: [{
                    Effect: "Allow",
    ...
    getClusterEncryptionKey(name: string, args: ClusterArgs): pulumi.Input<string> {
    ...
    createClusterAdmins(name: string, principalArn: pulumi.Input<string>, partition: pulumi.Output<string>, tags: Tags | undefined): pulumi.Output<string> {
        const clusterCreatorAdminPermissions = new aws.eks.AccessEntry(name, {
            clusterName: this.cluster.name,
        ...
        const accessPolicyAssociation = new aws.eks.AccessPolicyAssociation(name, {
            clusterName: this.cluster.name,
            ...
            parent: this,
            dependsOn: [this.cluster, clusterCreatorAdminPermissions],
        });
...
export function getKubeConfig(clusterName: pulumi.Input<string>, opts?: pulumi.InvokeOptions): pulumi.Output<string> {
    const cluster = aws.eks.getClusterOutput({
        name: clusterName,
    }, opts);

    return pulumi.output(cluster.certificateAuthorities.apply(certificateAuthorities => {
